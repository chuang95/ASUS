...
#define sBuildDate "2009-06-24"
#define sRevision "1.00d"
			...
			if(_stricmp(argv[i + 1], "/test")== 0)
			{
				...
				while(1)
				{
					  ...
					  cRecvBuff[nbSize] = '\0';
						...
						DWORD *p;
						p=(PDWORD)cRecvBuff;
						if(dwLastValue<*p && inet_addr(ip) == saClient.sin_addr.S_un.S_addr )
						{
							dwLastValue=*p;
							lTimes++;
						}
					...
					if(l64CurrentTime>=l64StopTime)
					{
						Sleep(5);
						//printf("%d\n",lTimes);
						lTimes=lTimes*8/1024/g_T*2;
						printf("upload speed is %ld Mbps\n",gUploadspeed);
						printf("download speed is %ld Mbps\n",lTimes);
